...
class AzureRepoPullRequestCommentLoader(BaseLoader):
    ...
    def load(self) -> List[Document]:
        ...
        for thread in response["value"]:
            ...
            line = thread["threadContext"]["rightFileStart"]["line"] if "rightFileStart" in thread["threadContext"]["rightFileStart"] else 0

            if file_path is not None and line > 0:
                for comment in thread["comments"]:
                    content = comment["content"] if "content" in comment else None
                    if content is not None:
                        docs.append(Document(content, {"file_path": file_path, "line": line}))
        return docs
